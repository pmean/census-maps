# This program reads redistricting files from the U.S. Census
# There are some minor modifications, but the bulk of the
# program can be found at 
#  https://www.census.gov/programs-surveys/decennial-census/about/rdo/summary-files.html
# under the link "R statistical software import scripts"
# The datasets used by this program were downloaded from
# the same page under the link "Legacy format summary files" 
#
# Decoding GEOID is tricky.
# The first seven digits describe the level of the GEOID.
#   Summary level (3 digits)
#   Geographic variant (2 digits)
#   Geographic component (2 digits)
#  For census blocks, you need to specify
#   State (2 digit FIPS code)
#   County (3 digit FIPS code)
#   Tract (6 digit Census code)
#   Block (4 digit Census code)
# Values for block groups, tracts, and entire counties will be shorter.
# Note that the single digit block group code is incorporated as the 
# first digit in the block code.
# These details are found at
#   https://www.census.gov/programs-surveys/geography/guidance/geo-identifiers.html
# The numeric variables of interest are
#  P0010001-Total population
#  H0010001-Total housing units
#  H0010002-Occupied housing units
#  H0010001-Vacant housing units
# If you want race counts, the key variables would be 
...
# If you want race/ethnicity counts, the key variables would be
#  P0020002  Hispanic or Latino
...
# where the variables White alone, etc. exclude Hispanic/Latino

# -----------------------------
# Specify location of the files
# -----------------------------
header_file_path <- "data/mogeo2020.pl"
part1_file_path  <- "data/mo000012020.pl"
part2_file_path  <- "data/mo000022020.pl"
part3_file_path  <- "data/mo000032020.pl"
...
# Here is code borrowed from another program
...
# Here is some original code
#
# Some investigation showed that the summary level
# (first three digits of GEOID) is 
#   "750" for census blocks
#   "150" for block groups
#   "140" for tracts

summary_level <- str_sub(combine$GEOID, 1, 3)
block <- summary_level=="750"
geoid1 <- str_sub(combine$GEOID[block], 10, 37)
geoid2 <- mo_census_blocks$GEOID20
length(geoid1)
length(geoid2)
sum(geoid1 %in% geoid2)
sum(geoid2 %in% geoid1)

file_name <- "tl_2020_29_tract.shp"
path_name                              %>%
  paste(file_name, sep="/")            %>%
  st_read(stringsAsFactors=FALSE)      -> mo_tracts
tract <- summary_level=="140"
geoid3 <- str_sub(combine$GEOID[tract], 10, 37)
geoid4 <- mo_tracts$GEOID
length(geoid3)
length(geoid4)
sum(geoid3 %in% geoid4)
sum(geoid4 %in% geoid3)

file_name <- "cb_2020_29_bg_500k.shp"
path_name                              %>%
  paste(file_name, sep="/")            %>%
  st_read(stringsAsFactors=FALSE)      -> mo_block_groups
block_group <- summary_level=="150"
geoid5 <- str_sub(combine$GEOID[block_group], 10, 37)
geoid6 <- mo_block_groups$GEOID
length(geoid5)
length(geoid6)
sum(geoid5 %in% geoid6)
sum(geoid5 %in% geoid6)
